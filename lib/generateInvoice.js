...
// Font config for proper Rs rendering and better Unicode support
...
async function ensureUnicodeFont(doc) {
    ...
    try {
        ...
        // If font fails to load, fallback to core font (Rs may not render)
    }
}

// helpeRs
const formatInr = (n) => `Rs${Number(n || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
...
export const generateInvoice = async (order) => {
    ...
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.setFontSize(18);
    doc.text('INVOICE', pageWidth - margin.right, y + 6, { align: 'right' });
    doc.setFont(UNICODE_FONT_NAME, 'normal');
    doc.setFontSize(9);
    doc.text(`#${orderIdShort}`, pageWidth - margin.right, y + 12, { align: 'right' });
